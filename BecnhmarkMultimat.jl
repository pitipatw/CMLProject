...
begin
    ...
    # objective function -> weigh minimization
    obj = y -> begin
...
begin
    ...
    ax3 = Axis(f2[1, 1])
    scatter!(ax3, mapping[:, 2], mapping[:, 1], color=dec_mat)


    ax4, hm1 = heatmap(f2[2,1],mapping[:, 2], mapping[:, 1], dec_mat)
    ax5, hm2 = heatmap(f2[3,1],mapping[:, 2], mapping[:, 1], dec_val)
